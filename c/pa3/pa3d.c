...
void InitRoad ()
{
    ...
    shm.queue[11] = Seminit(0);
    shm.queue[12] = Seminit(0);
    shm.eFlag = 0;
    shm.wFlag = 0;
    shm.wWaitCars = 0;
    shm.eWaitCars = 0;
	...
	int from, mph;
{
    ...
    if(shm.eCars == 0 && from == WEST){
        shm.wWaitCars += 1;
        Wait(shm.queue[11]);
        goto enterRoad;
    }else if(shm.wCars == 0 && from == EAST){
        shm.eWaitCars += 1;
        Wait(shm.queue[12]);
        goto enterRoad;
    }
    ...
    Signal (shm.queue[queue_tail]);
    PrintRoad ();
    Printf ("Car %d exits road\n", c);
    if(from == WEST){
        shm.wCars--;
        if(shm.eWait && shm.wCars == 0){
        ...
        else if(shm.wWaitCars > 0){
            Signal(shm.queue[11]);
            shm.wWaitCars -= 1;
        }
        else if(shm.wWaitCars == 0 && shm.eWaitCars == 0){
            shm.count -= 1;
        } 
    }
    else if(from == EAST){
        shm.eCars--;
        if(shm.wWait && shm.eCars == 0){
            Signal(shm.queue[11]);
            shm.wWaitCars -= 1;
            if(shm.wWaitCars == 0){
                shm.wWait = 0;
